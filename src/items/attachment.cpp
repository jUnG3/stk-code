...
void Attachment::saveState(BareNetworkString *buffer) const
{
    ...
    uint8_t type = m_type | (((m_type == ATTACH_BOMB) && (m_previous_owner != NULL)) ? (1 << 6) : 0) | bit_7;
...
void Attachment::hitBanana(ItemState *item_state)
{
    ...
    if (add_a_new_item)
    {
        ...
        case ATTACH_PARACHUTE:
        {
            ...
            if (m_initial_speed <= 150)
                m_initial_speed = 150;
            break;
        }
        case ATTACH_ANVIL:
            set(ATTACH_ANVIL, stk_config->time2Ticks(kp->getAnvilDuration()) + leftover_ticks);
            // if ( m_kart == m_kart[0] )
            ...
            break;
        case ATTACH_BOMB:
            set(ATTACH_BOMB, stk_config->time2Ticks(stk_config->m_bomb_time) + leftover_ticks);
            break;
        default:
            break;
        } // switch
    ...
    case ATTACH_PARACHUTE:
    {
        ...
        if (m_kart->getSpeed() <= initial_speed *
                                      (kp->getParachuteLboundFraction() +
                                       f * (kp->getParachuteUboundFraction() - kp->getParachuteLboundFraction())))
        {
            m_ticks_left = -1;
        }
    }
...
void Attachment::updateGraphics(float dt)
{
    ...
    if (m_type != ATTACH_NOTHING)
    {
        ...
        bool is_shield = m_type == ATTACH_BUBBLEGUM_SHIELD ||
                         m_type == ATTACH_NOLOK_BUBBLEGUM_SHIELD;
        float wanted_node_scale = is_shield ? std::max(1.0f, m_kart->getHighestPoint() * 1.1f) : 1.0f;
    ...
    case ATTACH_BOMB:
    {
        ...
        {
            m_node->setCurrentFrame(m_node->getEndFrame() - m_node->getStartFrame() - 1 - time_left);
        }
        return;
    }
    default:
